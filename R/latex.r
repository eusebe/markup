...
header.latex <- function(x, caption = NULL, align = "r", include.rownames = FALSE) {
    j <- paste(rep(align, length = ncol(x)), collapse = "")
    if (include.rownames) {
        j <- paste("l", j, sep = "")
    }
...
##' @return 
##' @author David Hajage \email{dhajage@@gmail.com}
tab.latex <- function(x, format = "f", digits = 2, decimal.mark = ".", drop0trailing = FALSE, na.print = "", include.rownames = FALSE, include.colnames = FALSE, rownames = NULL, colnames = NULL, align = "r", strong = NULL, italic = NULL, caption = NULL, ...) {
